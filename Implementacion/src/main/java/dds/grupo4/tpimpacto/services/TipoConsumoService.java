...
public class TipoConsumoService extends BaseService<TipoConsumo, TipoConsumoRepository> {
    ...
    private void seedTiposDeConsumoParaAlcanceEmisionesDirectas() {
        Unidad M3 = unidadService.getBySimbolo("m3").orElse(null);
        Unidad LT = unidadService.getBySimbolo("lt").orElse(null);
        Unidad KG = unidadService.getBySimbolo("kg").orElse(null);
        ...
        combustibleConsumidoGasoil.setFactorDeEmision(new FactorDeEmision(new Cantidad(LT, 1)));
        ...
        combustibleConsumidoGNC.setFactorDeEmision(new FactorDeEmision(new Cantidad(LT, 1)));
        ...
        combustibleConsumidoNafta.setFactorDeEmision(new FactorDeEmision(new Cantidad(LT, 1)));
    ...
    private void seedTiposDeConsumoParaAlcanceEmisionesIndirectas() {
        Unidad KWH = unidadService.getBySimbolo("kWh").orElse(null);
        ...
        electricidad.setFactorDeEmision(new FactorDeEmision(new Cantidad(KWH, 1)));

        this.saveAll(Arrays.asList(electricidad));
    }

    private void seedTiposDeConsumoParaAlcanceOtrasEmisionesIndirectas() {
        Unidad KM = unidadService.getBySimbolo("km").orElse(null);
        Unidad KG = unidadService.getBySimbolo("kg").orElse(null);
